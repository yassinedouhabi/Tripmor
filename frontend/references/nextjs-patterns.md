...
export default async function TripsPage({ searchParams }) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold text-neutral-900">Explore Trips</h1>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 flex gap-3">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-10 w-28 animate-pulse rounded-full bg-neutral-200" />
        ))}
      </div>
...
export default function TripsError({ error, reset }) {
  return (
    <section className="mx-auto flex max-w-7xl flex-col items-center justify-center px-4 py-24 text-center">
      ...
      <p className="mt-2 text-neutral-500">We couldn't load the trips. Please try again.</p>
      <button
        onClick={reset}
        className="mt-6 rounded-full bg-black px-8 py-3 font-semibold text-white hover:bg-neutral-800"
      >
        Try Again
      </button>
    </section>
